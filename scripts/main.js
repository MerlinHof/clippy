...
function modifyHtml() {
  ...
  document.querySelectorAll("a[href^='#']").forEach((link) => {
    link.addEventListener("click", function (e) {
      ...
      const targetText = decodeURI(
        link.getAttribute("href").substring(1).toLowerCase().trim(),
      );
      let targetElement = null;
      document
        .querySelectorAll("h1, h2, h3, h4, h5, h6")
        .forEach(function (hTag) {
          const hText = hTag.textContent
            .toLowerCase()
            .trim()
            .replaceAll(" ", "-");
...
function showControlPanel() {
  ...
  DOM.select("saveButton").setContent(
    "<img class='buttonImage' src='assets/images/tick.png'>Update",
  );
...
DOM.select("saveButton").onClick(async () => {
  ...
  if (res !== true) {
    ...
    errorDialog.content =
      res == 0
        ? "Please give your Clip a title and a text"
        : "You have reached Clippys Upload limit. Please try again in a few hours.";
  ...
  content.append(
    DOM.create("div .urlPreviewContainer")
      .append(
        DOM.create(
          "img .copyImage #copyClipUrlIcon [src=/assets/images/copy.png]",
        ).onClick(() => {
      ...
      .append(
        DOM.create(
          `a .urlPreview #urlToClip [target=_blank] [href=${links.clip}]`,
        ).setText(links.clip),
      ),
  );
  content.append(
    DOM.create("t .dialogText").setText(
      "To edit or delete your Clip, use the control panel link. Keep this link private:",
    ),
  );
  content.append(
    DOM.create("div .urlPreviewContainer")
      .append(
        DOM.create(
          "img .copyImage #copyControlPanelUrlIcon [src=/assets/images/copy.png]",
        ).onClick(() => {
      ...
      .append(
        DOM.create(
          `a .urlPreview #urlToControlPanel [target=_blank] [href=${links.controlPanel}]`,
        ).setText(links.controlPanel),
      ),
  );
  content.append(
    DOM.create("t .dialogText").setText(
      "Make sure to save both links in a secure location. Otherwise you won't be able to access or manage your Clip after this window closes.",
    ),
  );
...
document.getElementById("deleteButton").addEventListener("click", () => {
  ...
  deleteDialog.content =
    "If you proceed, every bit of data connected to this Clip will be wiped from the server. This action cannot be undone!";
