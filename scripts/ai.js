...
export async function showFactCheck(clip, visually = true) {
   ...
   answerDialog.isAiFeatureDialog = true;
   answerDialog.content = DOM.create("div").append(DOM.create("t").setText(analysis)).append(DOM.create("br")).append(DOM.create("div #credibilityScore")).getFirstElement();
   ...
   if (visually) {
      ...
      DOM.select("credibilityScore").setText("Credibility of " + score + "% " + emoji);
   }
...
export async function generateFaq(clip) {
   ...
   for (let i = 0; i < faq.length; i++) {
      ...
      faqDiv.append(DOM.create("t .aiFaqElementEmojis").setText((faqElement["EMO"] || "").replaceAll(" ", "")));
